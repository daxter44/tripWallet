...
export class TripEffects {
  constructor(private actions: Actions, private storageService: StorageService, private store: Store<TripsState>) {}
...
//   ));
    ...
    loadData = createEffect(() => {
        return this.actions.pipe(
            ofType(TripActions.loadTripState),
            mergeMap(() => {
                return this.storageService.get('trips').pipe(
                    map((trip : trip[]) => {
                        return TripActions.loadTripStateSuccess({trip});
                    })
                );
            })
        )
